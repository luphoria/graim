...
import config from "../config";
  export async function runLintCommand(
    ...
    for(let i = 0; i < rooms.length; i++) {
      ...
      if(power_levels["users"][await client.getUserId()] < 50) {
        ...
        client.sendMessage(rooms[i], {
          ...
          formatted_body: "It seems I'm not properly configured in this room! I need to be a Moderator (power level 50) in all rooms to work!",
        });
      }
      if(power_levels["events"]["m.room.power_levels"] > 50) {
        ...
        client.sendMessage(rooms[i], {
          ...
          msgtype: "m.notice",
          format: "org.matrix.custom.html",
          formatted_body: "It seems this room is improperly configured! The permission \"<b>Change permissions</b>\" (<code>m.room.power_levels</code>) must be at Moderator (power level 50) or below so I can use it!",
        });
      }
    }
